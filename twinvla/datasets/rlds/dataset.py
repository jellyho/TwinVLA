...
from twinvla.datasets.rlds import obs_transforms, traj_transforms
from twinvla.datasets.rlds.utils import goal_relabeling, task_augmentation
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        return traj
    # if name == 'bc_z_gresearch':
    #     format = {'file_format': tfds.core.FileFormat.PARQUET}
    # else:
    #     format = None
    ...
    # elif name == 'ucsd_pick_and_place_dataset_converted_externally_to_rlds':
    #     dataset = dataset.filter(
    ...
    if isinstance(dataset_statistics, str):
        ...
        dataset_statistics = get_dataset_statistics(
            full_dataset,
            hash_dependencies=(
                str(builder.info).replace('/dataset', '/data1/OXE'),
                str(state_obs_keys),
    ...
    if action_normalization_mask is not None:
        ...
        # Apply the same normalization mask to proprio if it has the same dimension
        if "proprio" in dataset_statistics:
            if dataset_statistics["proprio"]["mean"].shape[-1] == len(action_normalization_mask):
...
def make_interleaved_dataset(
    ...
    *,
    ...
    if global_normalization:
        print('Use global norm')
        if not dataset_statistics_path:
            # Compute global q99 and q01 for action and proprio
            action_q99   = np.max([ds["action"]["q99"]   for ds in all_dataset_statistics.values()], axis=0)
            action_q01   = np.min([ds["action"]["q01"]   for ds in all_dataset_statistics.values()], axis=0)
            proprio_q99  = np.max([ds["proprio"]["q99"]  for ds in all_dataset_statistics.values()], axis=0)
            proprio_q01  = np.min([ds["proprio"]["q01"]  for ds in all_dataset_statistics.values()], axis=0)
        else:
            print('Loading from calculated dataset statistics,')
            statistics = load_statistics_from_json(dataset_statistics_path)
            for ds in statistics.values():
                if single_arm:
                    action_q99 = ds["action"]["q99"]
                    action_q01 = ds["action"]["q01"]
                    proprio_q99 = ds["proprio"]["q99"]
                    proprio_q01 = ds["proprio"]["q01"]
                else:
                    if ds['action']['q99'].shape[0] > 15: # Finetuning TwinVLA-FT
                        action_q99 = ds["action"]["q99"]
                        action_q01 = ds["action"]["q01"]
                        proprio_q99 = ds["proprio"]["q99"]
                        proprio_q01 = ds["proprio"]["q01"]
                    else: # Training TwinVLA
                        action_q99 = np.concatenate([ds["action"]["q99"], ds["action"]["q99"]])
                        action_q01 = np.concatenate([ds["action"]["q01"], ds["action"]["q01"]])
                        proprio_q99 = np.concatenate([ds["proprio"]["q99"], ds["proprio"]["q99"]])
                        proprio_q01 = np.concatenate([ds["proprio"]["q01"], ds["proprio"]["q01"]])
                break # We assume all datasets have the same statistics
            
        # Bulk update every dataset with the computed values
        for ds in all_dataset_statistics.values():
            ds["action"].update({"q99": action_q99, "q01": action_q01})
            ds["proprio"].update({"q99": proprio_q99, "q01": proprio_q01})
